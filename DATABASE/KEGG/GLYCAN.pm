package DDB::DATABASE::KEGG::GLYCAN;
$VERSION = 1.00;
use strict;
use vars qw( $AUTOLOAD $obj_table $obj_table_g2p $obj_table_g2e $obj_table_g2r );
use Carp;
use DDB::UTIL;
{
	$obj_table = "$ddb_global{commondb}.kegg_glycan";
	$obj_table_g2r = "$ddb_global{commondb}.kegg_glycan2reaction";
	$obj_table_g2e = "$ddb_global{commondb}.kegg_glycan2enzyme";
	$obj_table_g2p = "$ddb_global{commondb}.kegg_glycan2pathway";
	my %_attr_data = (
		_id => ['','read/write'],
		_entry => ['','read/write'],
		_name => ['','read/write'],
...
sub add_reaction {
	...
	my $sth = $ddb_global{dbh}->prepare("INSERT IGNORE $obj_table_g2r (glycan,reaction,insert_date) VALUES (?,?,NOW())");
	for my $part (@parts) {
		if ($part =~ /^R\d+$/) {
...
sub add_pathway {
	...
	confess "No entry\n" unless $self->{_entry};
	$data =~ s/^PATHWAY\s*//;
	my $sth = $ddb_global{dbh}->prepare("INSERT IGNORE $obj_table_g2p (glycan,pathway,insert_date) VALUES (?,?,NOW())");
...
sub add_enzyme {
	...
	$data =~ s/^ENZYME\s*//;
	my @parts = split /\s+/, $data;
	my $sth = $ddb_global{dbh}->prepare("INSERT IGNORE $obj_table_g2e (glycan,enzyme,insert_date) VALUES (?,?,NOW())");
	for my $part (@parts) {
		if ($part =~ /^[0-9_\-\.]+$/) {
			$sth->execute($self->{_entry},$part );
		} elsif ($part =~ /^\(C\)$/) {
...
sub get_ids {
	...
	for (keys %param) {
		...
		if ($_ eq 'sequence_key') {
			...
			$join = "INNER JOIN $obj_table_g2p ON tab.id = glycan_key";
			push @where, sprintf "%s = %d", $_, $param{$_};
		} else {
			confess "Unknown: $_\n";
		}
...
sub exists {
	my($self,%param)=@_;
	if (ref($self) =~ /DDB::DATABASE::KEGG::GLYCAN/) {
		confess "No entry\n" unless $self->{_entry};
...
sub update_database {
	...
	my $log = '';
	my $filename = sprintf "%s/glycan", $param{directory};
	...
	while (my $line = <IN>) {
		...
		if ($read_mode eq 'ENTRY') {
			confess "Defined\n" if defined $GLYCAN;
			$GLYCAN = DDB::DATABASE::KEGG::GLYCAN->new( entry => (split /\s+/, $line)[1] );
		} elsif ($read_mode eq 'NAME') {
			$GLYCAN->add_to_name( $line );
		} elsif ($read_mode eq 'COMPOSITION') {
			$GLYCAN->add_to_information( $line );
			...
			$GLYCAN->add_to_information( $line );
		} elsif ($read_mode eq 'NODE') {
			$GLYCAN->add_to_information( $line );
		} elsif ($read_mode eq 'EDGE') {
			$GLYCAN->add_to_information( $line );
		} elsif ($read_mode eq 'REACTION') {
			$GLYCAN->add_reaction($line);
		} elsif ($read_mode eq 'PATHWAY') {
			$GLYCAN->add_pathway($line);
		} elsif ($read_mode eq 'ENZYME') {
			$GLYCAN->add_enzyme($line);
		} elsif ($read_mode eq '///') {
			eval {
				$GLYCAN->addignore_setid();
			};
			confess sprintf "%s\n%s\n",$tmp_log,$@ if $@;
			# reset variables
			undef $GLYCAN;
